...
type SuiteReport = {
  suite: "intervaltype-catalog-healthcheck";
  startedAt: string;
  finishedAt: string;
  summary: {
    total: number;
...
const fetchCatalog = async (): Promise<CatalogItem[]> => {
  const url = "https://intervaltype.com/wp-json/wp/v2/product?per_page=100&page=1&_fields=slug,link,title";
  ...
  if (!res.ok) {
    throw new Error(`Interval Type catalog fetch failed (${res.status})`);
  }
  const json = await res.json();
  if (!Array.isArray(json)) {
    throw new Error("Interval Type catalog response is not an array.");
  }
  ...
  for (const item of json) {
    ...
    if (!/^https:\/\/intervaltype\.com\/product\/[a-z0-9-]+\/?$/i.test(link)) continue;
...
const runCase = async (item: CatalogItem): Promise<CaseResult> => {
  ...
  try {
    const scraped = await IntervalTypeScraper.scrape(item.link);
    ...
    const specimenPdfUrls = Array.isArray(targetProfile.specimenPdfUrls)
      ? targetProfile.specimenPdfUrls
      : Array.isArray((scraped.metadata as any)?.specimenPdfUrls)
        ? (scraped.metadata as any).specimenPdfUrls
        : [];
    ...
    const status: CaseResult["status"] =
      reasons.length === 0 ? "pass" : reasons.some((r) => /mismatch|0 fonts/i.test(r)) ? "fail" : "warn";
...
async function run() {
  ...
  if (catalog.length === 0) {
    throw new Error("Interval Type catalog returned 0 products.");
  }
  ...
  const reportPath = path.join(reportsDir, `intervaltype-catalog-healthcheck-${toReportTimestamp(startedAt)}.json`);
  await writeFile(reportPath, JSON.stringify(report, null, 2), "utf8");

  console.log(`Interval Type catalog report: ${path.relative(process.cwd(), reportPath)}`);
...
run().catch((error) => {
  console.error("Interval Type catalog healthcheck failed:", error);
  process.exitCode = 1;
});
