...
const TYPEJI_STORE_GRAPHQL = "https://store.typeji.com/graphql";
const TYPEJI_FETCH_TIMEOUT_MS = 25000;
const TYPEJI_FETCH_MAX_RETRIES = 3;
const TYPEJI_UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36";
const TYPEJI_FEATURE_TAG_RE =
...
const normalizeTargetUrl = (rawUrl: string): string => {
  ...
  if (parsed.hostname.toLowerCase() === "typeji.com") {
    parsed.hostname = TYPEJI_HOST;
  }
  return parsed.href;
};
...
const fetchTextWithRetry = async (
  ...
  timeoutMs = TYPEJI_FETCH_TIMEOUT_MS
): Promise<string> => {
  let lastError: unknown;
  for (let attempt = 1; attempt <= TYPEJI_FETCH_MAX_RETRIES; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      if (attempt < TYPEJI_FETCH_MAX_RETRIES) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Typeji fetch failed");
};
...
const fetchGraphQlWithRetry = async <T>(params: {
  ...
  const endpoint = `${TYPEJI_STORE_GRAPHQL}?queryName=${encodeURIComponent(queryName)}`;
  let lastError: unknown;

  for (let attempt = 1; attempt <= TYPEJI_FETCH_MAX_RETRIES; attempt += 1) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), TYPEJI_FETCH_TIMEOUT_MS);
    try {
      const response = await fetch(endpoint, {
        method: "POST",
        signal: controller.signal,
        headers: {
          "User-Agent": TYPEJI_UA,
          Accept: "application/json",
          "Content-Type": "application/json",
          Origin: TYPEJI_ORIGIN,
          Referer: referer
        },
      ...
      throw new Error("Typeji GraphQL empty data payload");
    } catch (error) {
      lastError = error;
      if (attempt < TYPEJI_FETCH_MAX_RETRIES) {
        await sleep(500 * attempt);
  ...
  console.warn(`[TypejiScraper] GraphQL ${queryName} failed:`, lastError);
  return undefined;
};
...
const extractCollectionCssUrlsFromHtml = (html: string, baseUrl: string): string[] => {
  const out = new Set<string>();
  for (const match of html.matchAll(/<link[^>]+href=["']([^"']*fonts\.fontdue\.com\/typeji\/css\/[^"']+?\.css[^"']*)["'][^>]*>/gi)) {
    const raw = asString(match[1]);
    if (!raw) continue;
    try {
...
const collectFeatureTagsFromUnknown = (value: unknown, out: Set<string>): void => {
  if (typeof value === "string") {
    for (const match of value.matchAll(TYPEJI_FEATURE_TAG_RE)) {
...
const buildTypejiTargetProfile = (params: {
  ...
  return {
    profileId: "typeji-target-profile-v1",
    source: "typeji-html+fontdue-graphql+fontdue-css",
    foundry: "Typeji",
...
export const TypejiScraper: Scraper = {
  id: "typeji",
  name: "Typeji Precision Scraper",

  canHandle(url: string): boolean {
    return /(^|\/\/)(www\.)?typeji\.com/i.test(url);
  },
  ...
  async scrape(url: string): Promise<ScrapeResult> {
    try {
      ...
      const fallbackFamily = slug ? toTitleFromSlug(slug) : "Typeji";
      ...
      const specimenPdfUrls = extractSpecimenPdfUrls(html, targetUrl);
      ...
      const [storeData, characterData] = await Promise.all([
      ...
      const cssUrl =
        storeProfile?.cssUrl ||
        characterProfile?.cssUrl ||
        matchCssUrlByCollectionId(cssLinks, collectionId) ||
        cssLinks[0];

      let cssFaces: TypejiCssFace[] = [];
      if (cssUrl) {
        try {
          const cssText = await fetchTextWithRetry(cssUrl, {
            "User-Agent": TYPEJI_UA,
            Accept: "text/css,*/*;q=0.1",
            Referer: TYPEJI_ORIGIN,
            Origin: TYPEJI_ORIGIN
          });
          cssFaces = parseCssFaces(cssText, cssUrl, familyName);
        } catch (error) {
          console.warn("[TypejiScraper] CSS fetch failed:", error);
        }
      }
      ...
      console.error("[TypejiScraper] Error:", error);
      return {
        scraperName: this.name,
        foundryName: "Typeji",
        fonts: [],
