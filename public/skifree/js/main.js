...
// ── Win95 HUD DOM references ────────────────────────────────────────────────
const hudControls  = document.getElementById('sf-controls');
const hudDistance  = document.getElementById('sf-distance');
const hudLives     = document.getElementById('sf-lives');
const hudScore     = document.getElementById('sf-score');
const hudSpeed     = document.getElementById('sf-speed');
const gameoverEl   = document.getElementById('sf-gameover');
...
function renderHUD(speed) {
	if (hudDistance) hudDistance.textContent = `Travelled ${state.distanceTravelled}m`;
	if (hudLives)    hudLives.textContent    = `Skiers left: ${state.livesLeft}`;
	if (hudScore)    hudScore.textContent    = `High Score: ${state.highScore}`;
	if (hudSpeed)    hudSpeed.textContent    = `Current Speed: ${speed}`;
}
...
function startNeverEndingGame() {
	...
	// InfoBox instance retained for game logic compatibility; draw() is a no-op.
	// Win95 HUD rendering happens via the HTML overlay in index.html.
	const infoBox = new InfoBox({
		initialLines: [],
		position: { top: 15, right: 10 },
	});

	if (hudControls) hudControls.textContent = infoBoxControls;
